...
data <- read_mtrackj(folder = "motility/regulated",
...
data$id <- factor(data$id, levels = c("0 &micro;M",
                                      "1 &micro;M Dani",
                                      "10 &micro;M Dani",
                                      "100 &micro;M Dani",
                                      "1 &micro;M OM",
...
## data <- data[date != "2023-09-22" | pca != 4]
data <- data[date != "2023-10-12" | pca != 9]
...
## data_om <- fread("motility/regulated-om/data-by-id-om.csv")
## data_om_norm <- fread("motility/regulated-om/data-norm-om.csv")
## pca_lines_om <- fread("motility/regulated-om/pca-lines-om.csv")
## pca_lines_om_norm <- fread("motility/regulated-om/pca-lines-norm-om.csv")



## data_1uM <- fread("motility/reg-1uM-drugs/data-by-id-1uM.csv")
## data_1um_norm <- fread("motility/reg-1uM-drugs/data-norm-1uM.csv")
## pca_lines_1uM <- fread("motility/reg-1uM-drugs/pca-lines-1uM.csv")
## pca_lines_1uM_norm <- fread("motility/reg-1uM-drugs/pca-lines-norm-1uM.csv")

## data_by_id2 <- rbind(data_by_id, data_om, data_1uM)
## data_norm2 <- rbind(data_norm, data_om_norm, data_1um_norm)
## pca_lines2 <- rbind(pca_lines, pca_lines_om, pca_lines_1uM)
## pca_lines_norm2 <- rbind(pca_lines_norm, pca_lines_om_norm, pca_lines_1uM_norm)

## data_by_id2$id <- factor(data_by_id2$id, levels = c("0 &micro;M",
##                                                     "10 &micro;M",
##                                                     "dani-1uM",
##                                                     "10uM-OM",
##                                                     "om-1uM"))
...
## data_norm2$id <- factor(data_norm2$id, levels = c("0 &micro;M",
##                                                     "10 &micro;M",
##                                                     "dani-1uM",
##                                                     "10uM-OM",
##                                                     "om-1uM"))
...
ggplot()+
  ...
  geom_point(data = data_by_id[id %in% c("0 &micro;M", "10 &micro;M Dani")],
             aes(pca, speed_nm_s, color = id),
             size = 1.5)+
  geom_line(data = pca_lines[id %in% c("0 &micro;M", "10 &micro;M Dani")],
            aes(pca, Prediction, color = id),
            linewidth = 0.8)+
  scale_x_reverse()+
  ## facet_wrap(~id)+
...
saveRDS(gg_speed2, "figures/gg_mot_regulation.rds")

(gg_norm_speed2 <-
ggplot()+
  geom_errorbar(data = data_norm[id %in% c("0 &micro;M", "10 &micro;M Dani")],
                aes(pca,
                    ymin = norm_speed-sd_norm_speed,
                    ymax = norm_speed+sd_norm_speed,
                    color = id),
                width = 0.1)+
  geom_point(data = data_norm[id %in% c("0 &micro;M", "10 &micro;M Dani")],
             aes(pca, norm_speed, color = id),
             size = 1.5)+
  geom_line(data = pca_lines_norm[id %in% c("0 &micro;M", "10 &micro;M Dani")],
            aes(pca, Prediction, color = id),
            linewidth = 0.8)+
  scale_x_reverse()+
  ...
  xlab("pCa")+
  scale_color_manual(values = c("#666666", "#e7298a", alpha("#e7298a", 0.5), "#d95f02", alpha("#d95f02", 0.5)))+
  ## scale_color_manual(values = c("#666666", "#e7298a", "#d95f02"))+
  theme(
    ## axis.title.y = element_markdown(),
    legend.position = "none"
    )
...
(gg_norm_speed <-
ggplot()+
  geom_errorbar(data = data_norm2,
                aes(pca,
                    ymin = norm_speed-sd_norm_speed,
                    ymax = norm_speed+sd_norm_speed,
                    color = id),
                width = 0.1)+
  geom_point(data = data_norm2,
             aes(pca, norm_speed, color = id),
             size = 1.5)+
  geom_line(data = pca_lines_norm2,
            aes(pca, Prediction, color = id),
            linewidth = 0.8)+
  scale_x_reverse()+
  ## scale_y_continuous(breaks = seq(0, 1.25, by = 0.25))+
...
ggsave("supplemental-figures/regulated-motility-0-1-10-100-drugs.png", bg = "white")
