...
data <- read_mtrackj(folder = "motility/unreg-om",
                     id_split = c("mot", "unreg", "date", "id", "video", "filename"),
                     type = "Tracks")

data$id <- sub("uM-OM", " &micro;M", data$id)
...
ggplot()+
  ...
  ##          fill = NA,
  ##          label.color = NA )+
  draw_line(c(1, 2), c(300, 300))+
  annotate("text", x = 1.5, y = 315, label = paste0("p < 0.001 "), size = 3)+
...
ggsave("figures/gg_mot_om.png", bg = "white", dpi = 500)
