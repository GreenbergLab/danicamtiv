...
gg_example2 <-
  gg_example+
  coord_cartesian(xlim = c(-15, NA), ylim = c(-0.1, 1.1))+
  ## draw_line(x = c(-8, -8),
  ##           y = c(0.6, 0.8),
  ##           linetype = "solid",
  ##           linewidth = 0.8,
...
sim_dat$id <- ifelse(sim_dat$Dani == 0, "0 &micro;M", "10 &micro;M")
...
ggplot()+
  ...
  annotate("text", x = 1.5, y = 6.5, label = paste0("ns"), size = 3)+
...
ggatt2 <-
  ...
  annotate("text", x = 1.5, y = 0.0084, label = paste0(" p < 0.001 "), size = 3)+
...
bottom2 <- plot_grid(gg_transients, ggdet2, ggatt2, nrow = 1, rel_widths = c(1, 0.5, 0.5), labels = c("B", "C", "D"))

## ggsave("~/Downloads/detach-reattach-plot.pdf", bg = "white")


png("figures/for-the-boss2/figure-5.png", width = 6.5, height = 6, units = "in", res = 300)
plot_grid(gg_example2, bottom2, nrow = 2, rel_heights = c(1, 1.15), labels = c("A", ""))
dev.off()


pdf("figures/for-the-boss2/figure-5.pdf", width = 6.5, height = 5)
plot_grid(gg_example2, bottom2, nrow = 2, rel_heights = c(1, 1.15), labels = c("A", ""))
## plot_grid(gg_top, gg_bottom, nrow = 2, rel_heights = c(1.1, 1))
dev.off()

svg("figures/for-the-boss/detach-attach.svg", width = 6.5, height = 5)
plot_grid(gg_top, gg_bottom, nrow = 2, rel_heights = c(1.1, 1))
dev.off()
...
ggplot()+
  ...
  geom_jitter(data = data_nest[actin == "20uM-actin"],
              aes(x = id,
                  y = abs(coef_k2)),
              width = 0.1)+
  ylab("<i>k</i><sub>2</sub> (s<sup>-1</sup>)")+
  xlab("")+
  ggtitle("Rate of Re-Attachment")+
  scale_y_continuous(expand = expansion(c(0, 0.1), c(0, 0)))+
  scale_color_manual(values = colors)+
